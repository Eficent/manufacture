...
class MrpRepairOverhead(models.Model):
    _name = 'mrp.repair.overhead'
    ...
    def _prepare_account_move_line(self):
        ...
        for labor in self:
            ...
            if self.repair_id.company_id.currency_id.is_zero(debit_value) and not self.env['ir.config_parameter'].sudo().get_param('stock_account.allow_zero_cost'):
                raise ValidationError(_("The cost of %s is currently equal to 0. Change the cost or the overhead") % (self.product_id.display_name,))
        ...
        credit_line_vals = {
            ...
            'account_id': self.repair_id.type_id.credit_overhead_account_id.id,
        }
